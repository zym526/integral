...
Page({
  ...
  */
  ...
  switchSelect:function(){
    ...
    var selectIntegral=0;
    if (selectValue.length < 1) { 
      that.setData({ selectCountPrice: selectCountPrice,selectIntegral:selectIntegral,isGry:false }); 
    }else{
      for (var index in validList){
        if (that.inArray(validList[index].id, selectValue)){
          if(validList[index].productInfo.sale_type!=2){
            selectCountPrice = Number(selectCountPrice) + Number(validList[index].cart_num) * Number(validList[index].truePrice)
          }
          if(validList[index].productInfo.sale_type!=0){
            selectIntegral = Number(selectIntegral) + Number(validList[index].cart_num) * Number(validList[index].productInfo.attrInfo.integral)
          }
        }
      }
      if(that.data.myIntegral<selectIntegral){
        that.setData({
          isGry:true
        })
        app.Tips({ title: '积分不足' });
    ...
    getCartList().then(res=>{
      ...
      if (valid.length > 0) {
        for (var index in valid) {
          ...
          // valid[index].checked = true;
          // selectValue.push(valid[index].id);取消全部选中
        }
      }
      that.setData({ 
        cartList: cartList, 
