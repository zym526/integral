 ...
 */
    ...
    postOrderComputed(this.data.orderKey,{
      ...
      if (result){
        this.setData({ 
          ...
          integral: result.SurplusIntegral,
  ...
  */
   ...
   * 使用积分抵扣
  */
  ChangeIntegral1: function () {
    this.setData({ useIntegral1: !this.data.useIntegral1 });
  },
  /**
   * 选择地址后改变事件
   * @param object e
  ...
  */
  getConfirm:function(){
    ...
    orderConfirm(this.data.cartId).then(res=>{
      ...
      var integral_All=0;
      that.data.cartInfo.forEach(item=>{
        integral_All=app.help().Add(integral_All,app.help().Mul(item.cart_num,item.productInfo.attrInfo.integral))
      })
      that.setData({
        integral_All:integral_All,
      })
